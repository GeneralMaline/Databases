...
create table logs (
	-- id serial primary key, -- При попытке добавить идентфиикатор ключа ругается, мол должен быть только 1 идентификатор первичного ключа, а он и так единственный.
	created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
for each row
begin
	insert into logs (tbl_name, name)
...
-- а по теме НоСКуЛ вообще не понял как ДЗ делать.